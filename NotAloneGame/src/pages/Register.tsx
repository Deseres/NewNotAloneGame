  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (email.trim() && password.trim()) {
      try {
        ...
        // Ошибка обрабатывается в authStore
      }
    }
  };
        ...
        <div className="bg-slate-900/80 backdrop-blur-md p-8 rounded-2xl border border-slate-700/50 shadow-2xl w-full transition-all duration-500 hover:border-slate-500/50">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
              ...
              className="w-full bg-slate-700 hover:bg-slate-600 text-white font-black py-4 rounded-xl shadow-lg shadow-slate-900/20 transition-all duration-300 transform hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed uppercase tracking-widest cursor-pointer"
              style={{ fontFamily: "'Orbitron', sans-serif" }}
            >
